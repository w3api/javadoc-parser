...
__OUT__ = "/Users/victor/github/w3api_actual/Java/"
__OUTJSON__ = "/Users/victor/github/w3api/_data/Java/"
    ...
    for fichero in directorio:
        ...
        if fichero != ".DS_Store":
            ...
            try:
                ...
                if ((not "." in nombre) or ((nombre[nombre.find(".")+1].isupper()) and (nombre[nombre.find(".")+2].islower())) and (not "." in nombre[nombre.find(".")+1:])):
                    ...
                    metodo = nombre[nombre.rfind(".")+1:]  # El último punto (por si hay 2)
...
def generar_json():
    ...
    for enlace in enlaces:
            ...
            if descripcion!="" or codigo!="" or len(links) > 0:
                ...
                nombre = enlace.text[enlace.text.find(":")+1:].replace("(","").replace(")","").replace(" ","_") #Hay espacios que deberian de ser _
                data_json = {}


                # Campos. Si tuene un punto

                if tipo[:tipo.find(".")] == tipo[tipo.find(".")+1:]:
                        data_json["tipo"] = "constructores"
                else:
                    if not "(" in tipo:
                        data_json["tipo"] = "campos"
                    else:
...
URL = 'http://w3api.com/wiki/Categor%C3%ADa:Java'
